...
| 插件中心留在栈上 | 把插件中心从 `dsh.profile.bundles` 里拿掉，或写进 `dsh.profile.disabled`，会被当成卸载或停用——这两件事插件中心都不接受。下次启动会把它放回去。其余交付的 bundle 仍可通过 Disable 离开组装栈。通过插件中心做的 Update 会写成 profile 的真实依赖，此后外壳不再动那份副本。store 仍然记着提供过什么，好让以后的构建能摘掉自己不再带的那一行。 |
