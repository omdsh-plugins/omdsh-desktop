...
| 插件中心和模式系统留在栈上 | 把其中任何一个从 `dsh.profile.bundles` 里拿掉，或写进 `dsh.profile.disabled`，会被当成卸载或停用——这两件事它们都不接受。下次启动会把它放回去。通过插件中心做的 Update 会写成 profile 的真实依赖，此后外壳不再动那份副本。store 仍然记着提供过什么，好让以后的构建能摘掉自己不再带的那一行。 |
| 新的打包版本 | 打包应用换了一个这个外壳还没为这个 home 准备过的版本时，第一次启动会删掉 `$DSH_HOME/profiles`，然后再 seeding。设置、凭证和会话留着。同一个版本之后的启动，以及每一次从检出运行，都不会动 profile。 |
