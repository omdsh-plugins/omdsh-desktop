...
| The hub stays on | Taking the hub out of `dsh.profile.bundles`, or parking it on `dsh.profile.disabled`, is treated as an uninstall or a disable — neither of which the hub accepts. The next launch puts it back. Other shipped bundles can still leave the stack through Disable. An Update through the hub writes a real profile dependency, and this shell then leaves that copy alone. The store still records what was offered so a later build can drop a row it no longer ships. |
