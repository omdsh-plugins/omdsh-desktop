...
| The hub and the mode system stay on | Taking either out of `dsh.profile.bundles`, or parking it on `dsh.profile.disabled`, is treated as an uninstall or a disable — neither of which they accept. The next launch puts them back. An Update through the hub writes a real profile dependency, and this shell then leaves that copy alone. The store still records what was offered so a later build can drop a row it no longer ships. |
| A new packaged version | The first launch of a packaged application whose version this shell has not prepared the home for deletes `$DSH_HOME/profiles` and then seeds. Settings, credentials, and sessions stay. Later launches of the same version, and every checkout run, leave the profile alone. |
...
- Every Windows setup removes the default `~/.dsh/profiles` during installation, including a same-version overwrite. It leaves every sibling untouched, so settings, credentials, and sessions remain. The packaged shell's first-launch cleanup remains the fallback for a custom `$DSH_HOME` and for a setup-time deletion that could not complete.
- Closing the last window on Windows quits the application and stops the runtime; on macOS it does not.
- A new packaged version drops hub-installed plugins. The first launch after the version changes deletes `$DSH_HOME/profiles` so a leftover that cannot load against this runtime cannot take the boot down. Settings, credentials, and sessions stay; the hub is what puts plugins back.
