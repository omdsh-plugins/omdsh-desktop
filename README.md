...
The installer carries [`omdsh-plughub`](https://github.com/omdsh-plugins/omdsh-plughub) and [`omdsh-basemode`](https://github.com/omdsh-plugins/omdsh-basemode). Each is there for a different reason, and neither generalizes to a third.
...
- **Every Windows setup clears the default profile plugins.** Setup removes `~/.dsh/profiles` even for a same-version overwrite, while preserving the rest of `.dsh`. The first-launch check above remains the fallback for a custom `$DSH_HOME` or a setup-time cleanup failure.
